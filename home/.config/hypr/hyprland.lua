...
hl.on('hyprland.start', function()
    ...
    -- System
    hl.exec_cmd('dbus-update-activation-environment --systemd WAYLAND_DISPLAY XDG_CURRENT_DESKTOP HYPRLAND_INSTANCE_SIGNATURE XDG_SESSION_CLASS PATH')
    hl.exec_cmd('systemctl --user import-environment WAYLAND_DISPLAY XDG_CURRENT_DESKTOP HYPRLAND_INSTANCE_SIGNATURE XDG_SESSION_CLASS PATH')
...
--hl.animation({ leaf = "overviewIn",   enabled = true, speed = 5, bezier = "easeOutQuint" })
--hl.animation({ leaf = "overviewOut",  enabled = true, speed = 6, bezier = "quick" })
--hl.animation({ leaf = "overviewMove", enabled = true, speed = 7, bezier = "easeOutQuint" })
--hl.animation({ leaf = "overviewFade", enabled = true, speed = 7, bezier = "almostLinear" })
...
hl.bind(mainMod .. ' + Delete', hl.dsp.exec_cmd('~/.local/bin/sunshine-display-restore; hyprctl dispatch dpms on'), { locked = true, description = 'Restore laptop display and reload config' })
