...
-- Use the current file's Git root, falling back to the working directory.
...
-- Hide the terminal window without stopping its shell or losing scrollback.
local terminal_buf
local function toggle_terminal()
	...
	if terminal_buf and vim.api.nvim_buf_is_valid(terminal_buf) then
		...
		vim.fn.jobstart(vim.o.shell, {
			...
			cwd = cwd,
			on_exit = function()
				if terminal_buf == buf then terminal_buf = nil end
			end,
		})
	end
	vim.cmd("startinsert")
						...
						client.config.settings.Lua = vim.tbl_deep_extend("force", client.config.settings.Lua, {
							...
							workspace = {
								...
								library = vim.tbl_extend("force", vim.api.nvim_get_runtime_file("", true), {
									"${3rd}/luv/library",
									"${3rd}/busted/library",
									vim.fn.expand("/usr/share/hypr/stubs"),
								}),
							},
						})
					end,
		...
		config = function()
			...
			require("bufferline").setup({
				options = {
					...
					offsets = { { filetype = "neo-tree", text = "Files", separator = true } },
				},
			})
		end,
	},
