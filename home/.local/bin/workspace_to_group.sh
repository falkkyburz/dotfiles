...
workspace_address=$(hyprctl activeworkspace -j | jq -er '(.address // .id) | tostring')
...
while IFS=$'\t' read -r address x y; do
    ...
    jq -r --arg workspace_address "$workspace_address" '
        .[]
        | select(((.workspace.address // .workspace.id) | tostring) == $workspace_address and (.floating | not))
        | [
            .address,
            ((.at[0] + (.size[0] / 2)) | floor),
            ((.at[1] + (.size[1] / 2)) | floor)
