...
sanitize() {
  printf '%s' "$1" | perl -Mutf8 -CSDA -pe '
    ...
    s/[^\p{L}\p{N}\p{P}\p{Zs}\x{E000}-\x{F8FF}\x{F0000}-\x{FFFFD}\x{100000}-\x{10FFFD}~✓✗↓↑←→↔↕▲▼△▽⚠]/ /g;
...
monitor_workspace_title_rows() {
  local monitors clients workspaces
  ...
  jq -r --argjson clients "$clients" --argjson workspaces "$workspaces" '
    # Hyprland now identifies workspaces by address; older versions use id.
    def workspace_key: (.address // .id // "") | tostring;
    .[] as $mon |
    (
      $workspaces
      | map(select(workspace_key == ($mon.activeWorkspace | workspace_key)))
      | first
    ) as $workspace |
    (
      $clients
      | map(select(
          .monitor == $mon.id and
          (.workspace | workspace_key) == ($mon.activeWorkspace | workspace_key) and
          .visible == true and
          .hidden == false
        ))
    ...
    [
      ...
      (($mon.activeWorkspace.address // $mon.activeWorkspace.id // "?") | tostring),
...
cleanup() {
  ...
  if [[ -n "${idle_dbus_monitor_pid:-}" ]]; then
    kill "$idle_dbus_monitor_pid" 2>/dev/null || true
  fi

  if [[ -n "${idle_dbus_fd:-}" ]]; then
    exec {idle_dbus_fd}>&- 2>/dev/null || true
  fi
}
...
layout_icon() {
  ...
  printf '<span font_family="HyprLayout Icons">%s</span>' "$icon"
}
...
idle_inhibit() {
  local clients pid pidfile="${XDG_RUNTIME_DIR:-/tmp}/qt-idle-inhibit.pid"
  ...
  # Browsers normally inhibit hypridle through org.freedesktop.ScreenSaver.
  # That interface has no query for active inhibitors, so start_idle_dbus_monitor
  # follows its Inhibit/UnInhibit calls and keeps this count for us.
  if (( ${idle_dbus_count:-0} > 0 )); then
    printf '󰒳 '
    return 0
  fi

  # Also cover clients using the native Wayland idle-inhibit protocol.
  clients="$(hyprctl clients -j 2>/dev/null)" || return 0
  jq -e 'any(.[]; .inhibitingIdle == true)' <<< "$clients" >/dev/null 2>&1 || return 0

  printf '󰒳 '
}

declare -A idle_dbus_owner_counts=()
idle_dbus_count=0
idle_dbus_name_change_field=0
idle_dbus_old_owner=''

handle_idle_dbus_event() {
  local line="$1" owner value count

  if [[ "$line" == method\ call*'interface=org.freedesktop.ScreenSaver; member=Inhibit' ]]; then
    [[ "$line" =~ sender=([^[:space:]]+) ]] || return 0
    owner="${BASH_REMATCH[1]}"
    count="${idle_dbus_owner_counts[$owner]:-0}"
    idle_dbus_owner_counts[$owner]=$((count + 1))
    idle_dbus_count=$((idle_dbus_count + 1))
    return 0
  fi

  if [[ "$line" == method\ call*'interface=org.freedesktop.ScreenSaver; member=UnInhibit' ]]; then
    [[ "$line" =~ sender=([^[:space:]]+) ]] || return 0
    owner="${BASH_REMATCH[1]}"
    count="${idle_dbus_owner_counts[$owner]:-0}"
    if (( count > 0 )); then
  ...
  if [[ "$line" == signal\ *'interface=org.freedesktop.DBus; member=NameOwnerChanged' ]]; then
    idle_dbus_name_change_field=1
    idle_dbus_old_owner=''
    return 0
  fi

  (( idle_dbus_name_change_field > 0 )) || return 0
  [[ "$line" == '   string "'*'"' ]] || return 0
  value="${line#*string \"}"
  value="${value%\"}"
  case "$idle_dbus_name_change_field" in
    1) idle_dbus_name_change_field=2 ;;
    2)
      idle_dbus_old_owner="$value"
      idle_dbus_name_change_field=3
      ;;
    3)
      if [[ -z "$value" && -n "$idle_dbus_old_owner" ]]; then
        count="${idle_dbus_owner_counts[$idle_dbus_old_owner]:-0}"
        if (( count > 0 )); then
          idle_dbus_count=$((idle_dbus_count - count))
          unset 'idle_dbus_owner_counts[$idle_dbus_old_owner]'
        fi
      fi
      idle_dbus_name_change_field=0
      idle_dbus_old_owner=''
...
start_idle_dbus_monitor() {
  local match_inhibit match_uninhibit match_disconnect

  command -v dbus-monitor >/dev/null 2>&1 || return 1
  [[ -n "${DBUS_SESSION_BUS_ADDRESS:-}" ]] || return 1

  match_inhibit="type='method_call',interface='org.freedesktop.ScreenSaver',member='Inhibit'"
  match_uninhibit="type='method_call',interface='org.freedesktop.ScreenSaver',member='UnInhibit'"
  match_disconnect="type='signal',sender='org.freedesktop.DBus',interface='org.freedesktop.DBus',member='NameOwnerChanged',arg2=''"

  idle_dbus_fifo="$(mktemp -u "${TMPDIR:-/tmp}/run-minibar-idle-dbus.XXXXXX")"
  mkfifo "$idle_dbus_fifo"
  exec {idle_dbus_fd}<>"$idle_dbus_fifo"
  rm -f "$idle_dbus_fifo"

  dbus-monitor --session "$match_inhibit" "$match_uninhibit" "$match_disconnect" >&$idle_dbus_fd &
  idle_dbus_monitor_pid=$!
  return 0
}
...
pi=0
cpu() {
  local _ u n s i w irq sirq st g gn total dt di pct raw
...
# Nerd Font nf-md-expansion_card (U+F08AE), beside the CPU item.
gpu_s='󰢮  ?%'
start_gpu() {
  command -v minibar-gpu-load >/dev/null 2>&1 || return 0
  coproc GPU_READER { exec minibar-gpu-load; }
  gpu_pid=$GPU_READER_PID
  exec {gpu_fd}<&"${GPU_READER[0]}"
  exec {gpu_input_fd}>&"${GPU_READER[1]}"
}
...
gpu() {
  local raw='' value display
  if [[ -n "${gpu_fd:-}" ]] && kill -0 "$gpu_pid" 2>/dev/null; then
    for _ in {1..5}; do
      IFS= read -r -t 0.001 -u "$gpu_fd" value || break
      raw=$value
    done
    [[ -n "$raw" ]] || return 0
  fi
  if [[ "$raw" =~ ^[0-9]{1,3}$ ]] && ((10#$raw <= 100)); then
    raw=$((10#$raw))
    printf -v display '%2d' "$raw"
    if ((raw > 95)); then
      printf -v gpu_s '󰢮 <span foreground="%s">%s%%</span>' "$c_red" "$display"
    else
      printf -v gpu_s '󰢮 %s%%' "$display"
    fi
  else
    gpu_s='󰢮  ?%'
  fi
...
emit_for_monitors() {
  local rows mon visible ws_id layout title_s left right error_flag=''
  rows="${1:-}"
  (( ${hypr_error:-0} )) && error_flag="$(markup "$c_red" '✗')"

  if [[ -z "$rows" ]]; then
    right="$(join_nonempty "$pwr_s" "$usb_s" "$idle_s" "$(layout_icon)" "$cpu_s" "$gpu_s" "$mem_s" "$br_s" "$net_s" "$bt_s" "$vol_s" "$pow_s" "$bat_s")"
    printf '%s%s%s%s%s\n' \
  ...
  while IFS=$'\t' read -r mon visible ws_id layout title_s; do
    [[ -n "$mon" ]] || continue
    title_s="$(sanitize "$(short "$title_s" 40)")"
    left="$(join_nonempty "$error_flag" "$(ws_label "$ws_id")" "$title_s" "$med_s")"
    right="$(join_nonempty "$pwr_s" "$usb_s" "$idle_s" "$(layout_icon "$layout")" "$cpu_s" "$gpu_s" "$mem_s" "$br_s" "$net_s" "$bt_s" "$vol_s" "$pow_s" "$bat_s")"
...
bar_state=''
idle_dbus_events=0
if start_idle_dbus_monitor; then
  idle_dbus_events=1
fi
mem_s="$(mem)"
net
...
while true; do
  ...
  if (( idle_dbus_events )); then
    if ! kill -0 "$idle_dbus_monitor_pid" 2>/dev/null; then
      idle_dbus_events=0
    else
      for _ in {1..40}; do
        idle_dbus_event=''
        IFS= read -r -t 0.001 -u "$idle_dbus_fd" idle_dbus_event || break
        handle_idle_dbus_event "$idle_dbus_event"
      done
    fi
  fi
